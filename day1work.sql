...
SELECT * FROM orders WHERE customer_id IN (
  SELECT customer_id FROM customer WHERE active = false
);
...
FROM customer
WHERE EXISTS (
  SELECT 2 FROM orders WHERE orders.customer_id = customer.customer_id
);

-- Step 20: Update Statement
UPDATE customer
SET first_name = 'Margi', last_name = 'sachade', email = 'bansi.sachade@tatvasoft.com'
WHERE customer_id = 1;
